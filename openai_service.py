...
5. 如确有必要补充其他关键人物才能闭环（例如用户提供的关键人物<2个的时候）：可在 parties_structure 中加入“待补齐角色”（同样标注 role_type），并在 overview.suggested_roles_to_hunt 中给出【去哪找 + 话术】；但**只有当现有人物明显不足以构成闭环**时才建议补充；若已足够闭环，则该数组必须为 []。
